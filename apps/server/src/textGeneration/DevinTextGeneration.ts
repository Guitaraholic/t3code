...
export const makeDevinTextGeneration = Effect.fn("makeDevinTextGeneration")(function* (
  ...
  const runDevinJson = <S extends Schema.Top>({
    ...
    prompt,
    outputSchemaJson,
    // Accepted for parity with the other text-generation helpers, deliberately
    // unused: Devin's selectable models vary by plan, so a background helper
    // stays on whatever model the session already has.
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        // Deliberately no model selection: these helpers produce titles and
        // commit messages, and Devin's selectable models vary by plan. Letting
        // the session keep its own model avoids failing a background helper
        // over a model the account cannot use.

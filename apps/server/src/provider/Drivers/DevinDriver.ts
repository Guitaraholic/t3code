/**
 * DevinDriver — `ProviderDriver` for the Devin CLI (`devin acp`, Cognition).
 *
 * Devin exposes a plain ACP agent over stdio, so the shared ACP runtime does
 * the protocol work and this driver only assembles the per-instance bundle.
 *
 * The model catalog comes from the provider status check rather than from
 * settings, because the signed-in account's plan — not T3 — decides which
 * models are selectable. See {@link ../Layers/DevinProvider}.
 *
...
// The Devin CLI ships its own updater (`devin update`), so the resolved
// executable owns the update. No executable means nothing to update, not
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const checkProvider = checkDevinProviderStatus(effectiveConfig, processEnv, cwd).pipe(
        ...
        Effect.provideService(ChildProcessSpawner.ChildProcessSpawner, spawner),
        // The probe opens its scratch session in a temp directory rather than
        // the project, so it needs a file system to make one.
        Effect.provideService(FileSystem.FileSystem, fileSystem),
      );

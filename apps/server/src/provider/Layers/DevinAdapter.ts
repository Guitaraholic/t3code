...
/** Devin's session lock usually clears as the previous child finishes exiting. */
const DEVIN_SESSION_LOAD_TIMEOUT = "25 seconds";
const DEVIN_SESSION_LOCK_RETRIES = 4;
const DEVIN_SESSION_LOCK_RETRY_DELAY = "1500 millis";
...
/**
 * Merges T3's device-scoped MCP environment into the agent's, when the running
 * build has that seam.
 *
 * `withAgentDeviceEnvironment` arrived in T3 after this provider was written,
 * and this patch is carried across nightly releases rather than shipped with
 * one. Probing for it keeps the provider working on builds from either side of
 * that change instead of failing at session start; on older builds the agent
 * simply sees the plain environment.
 */
function resolveDevinSessionEnvironment(
  environment: NodeJS.ProcessEnv | undefined,
  mcpSession: unknown,
): NodeJS.ProcessEnv | undefined {
  const support = McpProviderSession as {
    readonly withAgentDeviceEnvironment?: (
      env: NodeJS.ProcessEnv,
      session: unknown,
    ) => NodeJS.ProcessEnv;
  };
  const deviceEnvironment = (mcpSession as { agentDeviceEnvironment?: unknown } | undefined)
    ?.agentDeviceEnvironment;
  if (!environment && !deviceEnvironment) {
    return undefined;
  }
  return typeof support.withAgentDeviceEnvironment === "function"
    ? support.withAgentDeviceEnvironment(environment ?? process.env, mcpSession)
...
function applyRequestedSessionConfiguration<E>(input: {
  ...
  /**
   * Devin answers from its own default model when handed one the account's
   * plan does not allow, logging only a stderr warning. Reporting that as a
   * turn failure is the whole point: a silent substitution would attribute
   * another model's output to the one the user picked.
   */
  readonly mapUnavailableModel: (context: {
  ...
  return Effect.gen(function* () {
    if (input.modelSelection) {
      ...
      if (unavailableModel) {
        return yield* Effect.fail(input.mapUnavailableModel(unavailableModel));
      }
...
export function makeDevinAdapter(devinSettings: DevinSettings, options?: DevinAdapterLiveOptions) {
  return Effect.gen(function* () {
    ...
    const startSessionAttempt: DevinAdapterShape["startSession"] = (input) =>
      withThreadLock(
        input.threadId,
        Effect.gen(function* () {
          ...
          const sessionEnvironment = resolveDevinSessionEnvironment(
            options?.environment,
            mcpSession,
          );
          const acp = yield* makeDevinAcpRuntime({
            ...
            cwd,
            runtimeMode: input.runtimeMode,
            // The shared default is 90s, chosen for agents that replay a long
            // history on load. An unloadable Devin session never answers at
            // all, so waiting the full 90s only delays the fallback below —
            // healthy sessions here load in well under a second.
            sessionLoadTimeout: DEVIN_SESSION_LOAD_TIMEOUT,
            ...(resumeSessionId ? { resumeSessionId } : {}),
            clientInfo: { name: "t3-code", version: "0.0.0" },
            // No MCP servers. T3 serves its toolkit over HTTP, and Devin
            // advertises `mcpCapabilities: {http: false, sse: false}` — it
            // speaks stdio MCP only, so an HTTP entry here is something it
            // cannot use. `mcpSession` is still read above because it also
            // carries the agent-device environment. Devin opens pull requests
            // with the `gh` CLI instead, and they are linked to the thread
            // from the client's own Link pull request dialog.
            ...acpNativeLoggers,
          }).pipe(
          ...
          const started = yield* Effect.gen(function* () {
            ...
            // Devin locks a session to one process. A restart can leave the
            // lock held by an exiting child, and T3 then resumes the thread
            // into a `-32015` that Devin itself marks retryable — so wait for
            // the corpse rather than failing the turn after a long stall.
            return yield* acp.start().pipe(
              Effect.retry({
                while: (error) => isDevinSessionLockedError(error),
                times: DEVIN_SESSION_LOCK_RETRIES,
                schedule: Schedule.spaced(DEVIN_SESSION_LOCK_RETRY_DELAY),
              }),
            );
          }).pipe(
          ...
          yield* applyRequestedSessionConfiguration({
            ...
            mapUnavailableModel: ({ requested, available }) =>
              new ProviderAdapterValidationError({
                provider: PROVIDER,
                operation: "session/set_config_option",
                issue:
                  `Devin cannot run "${requested}" on the signed-in account. ` +
                  (available.length > 0
                    ? `Models available now: ${available.join(", ")}.`
                    : "The agent reported no selectable models."),
              }),
          });
    ...
    const sendTurn: DevinAdapterShape["sendTurn"] = (input) =>
      Effect.gen(function* () {
        ...
        return yield* Effect.gen(function* () {
          ...
          const resolvedModel = model?.trim() || DEVIN_SESSION_DEFAULT_MODEL;
          yield* applyRequestedSessionConfiguration({
            runtime: ctx.acp,
              ...
              prompt: [
                ...promptParts,
                {
                  ...
                  text: buildRuntimeInstructions({
                    harness: "Devin",
                    model: resolvedModel,
                    // Devin reports mcpCapabilities {http:false, sse:false}: it
                    // speaks only stdio MCP, so it never sees T3's toolkit.
    ...
    /**
     * Resume if we can, start fresh if we cannot.
     *
     * A Devin session can end up unloadable — `session/load` never answers,
     * for Devin's own CLI as much as for T3 — and the thread then fails every
     * turn for ever, each attempt costing a full load timeout. The history in
     * such a session is already unreachable by any client, so nothing is lost
     * by abandoning it, while keeping the thread usable is worth a great deal.
     * Only a resume is retried: a first-time start that fails has no fallback
     * to offer and must surface its error.
     */
    const startSession: DevinAdapterShape["startSession"] = (input) =>
      startSessionAttempt(input).pipe(
        Effect.catchIf(
          (error) => input.resumeCursor !== undefined && isDevinSessionLoadFailure(error),
          (error) =>
            Effect.logWarning(
              "Devin session could not be resumed; starting a new one for this thread.",
              { threadId: input.threadId, error },
            ).pipe(Effect.andThen(startSessionAttempt({ ...input, resumeCursor: undefined }))),
        ),
      );
